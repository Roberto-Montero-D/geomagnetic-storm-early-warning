...
def _validate_development_folds(
    ...
    """Reject any development fold that touches the protected Final Test."""
    for fold in folds:
        for role, index in (
            ...
            if not index.isin(splits.index).all():
                raise ValueError(
                    f"{fold.name} {role} index contains timestamps absent from splits."
                )

            periods = splits.loc[index, "period"]
            if (periods == PERIOD_FINAL_TEST).any():
                raise ValueError(
                    f"{fold.name} {role} index touches the protected Final Test."
                )


def _aggregate_far(fold_rows: list[dict]) -> float:
    exposure = sum(row["valid_exposure_hours"] for row in fold_rows)
    false_alarms = sum(row["n_false_alarm_episodes"] for row in fold_rows)
...
def _select_cross_fold_threshold(
    ...
    *,
    ...
    """Select the minimum feasible Phase 2 baseline-evaluation threshold."""
    ...
    for threshold in thresholds:
        fold_rows = []
        for fold, prediction in zip(folds, predictions):
            probability = prediction.probabilities[baseline]
...
def evaluate_development_folds(
    ...
    *,
    ...
    _validate_development_folds(folds, splits)

    predictions = [
        generate_fold_baseline_predictions(dataset, fold)
        for fold in folds
    ]
    ...
    for baseline in PROBABILISTIC_BASELINES:
        threshold, table = _select_cross_fold_threshold(
            predictions,
            folds,
            events,
            baseline,
    ...
    for fold, prediction in zip(folds, predictions):
        for baseline in BASELINE_NAMES:
            threshold = selected[baseline]
            probability = prediction.probabilities[baseline]
            exposure = int(probability.notna().sum())
                ...
                metric_rows.append(
                    {
                        ...
                        "median_lead_time": pd.NaT,
                        "n_alert_episodes": 0,
                        "n_false_alarm_episodes": 0,
